...
- Ticket internal state mutators are no longer reachable from consumer code
  (there is no public `ticket._markDone()` etc.); only the client that
  created a ticket can settle it — consumers only ever see the public
  `Ticket` API (#27).
...
- Requests without `parse` are typed `Ticket<undefined>`. `client.get<User>(url)` used to type `result.data` as `User` while it was `undefined` at runtime; passing a type argument without `parse` is now a type error. Each method is overloaded: with `parse` it returns `Ticket<T>`, without it `Ticket<undefined>`, and `request()` also accepts a `parse` that may be `undefined` (`Ticket<T | undefined>`). New exported types: `ParsedRequestOptions<T>`, `UnparsedRequestOptions`.
- Removed from the public API: `createTicket` and `TicketController` (anyone holding them could settle a client's tickets), the `HttpClient#json()` method (use the standalone `json()` export), and `RequestOptions.url`, which was never read.
