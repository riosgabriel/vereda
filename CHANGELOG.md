...
- Bodies of unparsed success responses and `HttpError` responses are now bounded by the attempt's remaining `attemptMs` (capped at `totalMs`), measured from attempt start. Previously, once the ticket resolved with the raw `Response`, its body had no deadline at all — a server that sent headers then stalled the body could hang `raw.json()`/`error.response.text()` forever, past both timeouts. Streaming or reading a body later than that needs `timeout: { attemptMs: Infinity }` (and no `totalMs`) on that request, or your own `AbortSignal`.
