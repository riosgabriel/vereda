...
Vereda: resilient HTTP client for Node.js (queuing, retries, bulkhead isolation) built on global `fetch`. ESM-only, Node 22+, zero runtime dependencies. Published to npm with a prebuilt `dist/` (no install-time compile step); run `npx husky` once after cloning to wire up the local pre-commit hook.
...
- **Two test runtimes.** CI runs the suite under Node 22/24 *and* under Bun. `bun run --bun test` reproduces the Bun leg — without `--bun`, bun respects the vitest shebang and silently runs under Node. Runtime-conditional expectations (currently only the TRACE row in `test/core/retry-matrix.test.ts`) key off a `Bun` global check; CI sets `EXPECTED_RUNTIME` on both legs so a leg that silently changes runtime fails instead of passing.
